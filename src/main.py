...
class CameraSide:
    ...
    def save(self, mountPoint):
        ...
        if (
            ...
            and not isinstance(self.raw, basestring)
            and self.filename is not None
            and self.code == camera_thread.COMPLETE
        ):
...
def configureSides():
    if (
        odd.serial in config
        and "position" in config[odd.serial]
        and config[odd.serial]["position"] != "odd"
    ):
        swapSides()
    elif (
        even.serial in config
        and "position" in config[even.serial]
        and config[even.serial]["position"] != "even"
    ):
        swapSides()
    else:
        updateConfig()
...
def checkCameras():
    ...
    try:
        ...
        errorlog.write(
            "Failed to reset cameras: " + str(e.args) + "\n" + traceback.format_exc()
        )
...
def checkForDisconnected(manager):
    ...
    if (
        odd.code == camera_thread.DISCONNECTED
        or even.code == camera_thread.DISCONNECTED
    ):
...
class OptionSelect(RelativeLayout):
    ...
    def keyPress(self, key):
        handleKeyPress(key, {"1": self.done, "3": self.test})
    ...
    def test(self):
        try:
            ...
            odd.capture(
                "/debug/preview-odd", self.manager.mountPoint, camera_thread.AUTO_FOCUS
            )
            even.config[self.key] = self.evenControl.get()
            even.camera.isReady = False
            even.capture(
                "/debug/preview-even", self.manager.mountPoint, camera_thread.AUTO_FOCUS
            )
...
class SettingPicker(BoxLayout):
    ...
    def keyPress(self, key):
        handleKeyPress(
            key, {self.previousKey: self.goPrevious, self.nextKey: self.goNext}
        )
...
class StartScreen(Screen):
    ...
    def update(self, dt):
        ...
        if count == 0:
            self.syncWait = 0.0
            self.powerOff.text = (
                "All disks ejected. Ok to power off or disconnect disk."
            )
        else:
            ...
            if timeLeft > 0:
                waitString = (
                    "Syncing to disk. Waiting %0.0f more seconds before force ejecting."
                    % timeLeft
                )
            self.syncWait += dt
        ...
        if (
            (odd.camera is not None and odd.camera.is_connected())
            or (even.camera is not None and even.camera.is_connected())
        ) and not gphoto:
    ...
    def keyPress(self, key):
        handleKeyPress(
            key, {"1": self.beginAction, "2": self.turnOffCameras, "9": self.quitAction}
        )
...
class ConfigureDiskScreen(Screen):
    ...
    def update(self, dt):
        ...
        if len(sticks) == 0:
            ...
            if mountPoint is None:
                self.manager.mountPoint = None
                self.diskStatus.text = (
                    "Could not mount drive. Try removing and re-inserting it."
                )
                self.diskNext.disabled = True
                self.spinner.opacity = 1.0
    ...
    def keyPress(self, key):
        handleKeyPress(
            key,
            {"1": self.diskNextAction, "9": self.backAction, "2": self.upgradeAction},
        )
    ...
    def getUpgrade(self):
        ...
        for name in filenames:
            ...
            if match:
                ...
                if (
                    major > largestMajor
                    or major == largestMajor
                    and minor > largestMinor
                ):
                    largestMajor = major
                    largestMinor = minor
        if (
            largestMajor > versionMajor
            or largestMajor == versionMajor
            and largestMinor > versionMinor
        ):
            result = (
        ...
        if (
            ...
            if tooManyFound:
                self.cameraLabel.text = (
                    "Too many cameras found. Disconnect until there are only two."
                )
                ...
                self.cameraLabel.text = (
                    "No cameras found. Plug in or turn on the cameras." + errorText
                )
                self.cameraNext.disabled = True
                self.searchingLayout.opacity = 1.0
                self.foundLayout.opacity = 0.0
                ...
                self.cameraLabel.text = (
                    "Odd camera not found. Plug in or turn it on." + errorText
                )
                self.cameraNext.disabled = True
                self.searchingLayout.opacity = 1.0
                self.foundLayout.opacity = 0.0
                ...
                self.cameraLabel.text = (
                    "Even camera not found. Plug in or turn it on." + errorText
                )
                self.cameraNext.disabled = True
                self.searchingLayout.opacity = 1.0
                self.foundLayout.opacity = 0.0
...
class PreviewOutside(RelativeLayout):
    ...
    def moveUp(self):
        try:
            self.scatter.apply_transform(
                Matrix().translate(x=0, y=-100 / self.scatter.scale, z=0)
            )
        except Exception as e:
            handleCrash(e)
    ...
    def moveLeft(self):
        try:
            self.scatter.apply_transform(
                Matrix().translate(x=100 / self.scatter.scale, y=0, z=0)
            )
        except Exception as e:
            handleCrash(e)
...
class FocusCameraScreen(Screen):
    ...
    def on_pre_enter(self):
        try:
            ...
            if self.manager.newPreview:
                ...
                self.noPreviewLabel.opacity = 1.0
                if gphoto:
                    self.noPreviewLabel.text = (
                        "Press pages against glass and take a test shot"
                    )
                else:
                    self.noPreviewLabel.text = (
                        "Press pages against glass and tap refocus"
                    )
                self.preview.opacity = 0.0
    ...
    def cameraRefocusAction(self):
        try:
            odd.capture(
                "/debug/preview-odd", self.manager.mountPoint, camera_thread.LOCK_FOCUS
            )
            even.capture(
                "/debug/preview-even", self.manager.mountPoint, camera_thread.LOCK_FOCUS
            )
...
class CaptureWaitScreen(Screen):
    ...
    def keyPress(self, key):
        handleKeyPress(key, {"0": self.restartAction})
...
class CaptureScreen(Screen):
            ...
            if self.manager.newCapture:
                ...
                if (
                    ...
                    self.captureLabel.text = (
                        "Captured Pages "
                        + str(self.lastEvenPage)
                        + ", "
                        + str(self.lastEvenPage + 1)
                    )
    ...
    def scanAt(self, pageNumber):
        if pageNumber is not None:
            odd.capture(
                self.makeFile(pageNumber + 1),
                self.manager.mountPoint,
                camera_thread.KEEP_FOCUS,
            )
            even.capture(
                self.makeFile(pageNumber),
                self.manager.mountPoint,
                camera_thread.KEEP_FOCUS,
            )
...
class CaptureFailScreen(Screen):
    ...
    def updateLabel(self, label, side):
        if side.code == camera_thread.COMPLETE:
            ...
            label.text = (
                side.position + " camera: [color=ffaaaa]Failed[/color]: " + side.message
            )
    ...
    def keyPress(self, key):
        handleKeyPress(key, {"1": self.ok, "0": self.restartAction})
...
class DebugScreen(Screen):
    def update(self, dt):
        if (
            ...
            self.updateSide(
                evenFound, even, self.evenStatus, self.evenLog, self.evenMessage
            )
        if (
            even.camera is not None
            and even.camera.is_connected()
            and odd.camera
            and odd.camera.is_connected()
        ):
    ...
    def getLog(self, side):
        try:
            ...
            side.message = (
                "Could not get debug log: " + side.camera.message + ": " + str(e)
            )
        ...
        try:
            if not checkForCrash(self.manager):
                if not self.manager.hasTransitioned or (
                    self.manager.current_screen.transition_progress == 1
                    and self.manager.current_screen.transition_state == "in"
                ):
