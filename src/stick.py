...
def _search():
    ...
    for key, value in listDevices().items():
        try:
            if (
                "org.freedesktop.UDisks2.Block" in value
                and "org.freedesktop.UDisks2.Filesystem" in value
            ):
                block = value["org.freedesktop.UDisks2.Block"]
                drive = dbus.Interface(
                    bus.get_object("org.freedesktop.UDisks2", block["Drive"]),
                    "org.freedesktop.UDisks2.Drive",
                )
                driveprop = dbus.Interface(drive, "org.freedesktop.DBus.Properties")
                busType = driveprop.Get(
...
class Stick:
    ...
    def mount(self):
        ...
        try:
            if mount_point is None:
                bus = dbus.SystemBus()
                fs = dbus.Interface(
                    bus.get_object("org.freedesktop.UDisks2", self.path),
                    "org.freedesktop.UDisks2.Filesystem",
                )
                mount = fs.get_dbus_method(
                    "Mount", dbus_interface="org.freedesktop.UDisks2.Filesystem"
                )
                mount_point = mount([])
    ...
    def get_mount_point(self):
        ...
        try:
            ...
            old_mounts = fsprop.Get("org.freedesktop.UDisks2.Filesystem", "MountPoints")
            if len(old_mounts) > 0:
                mount_point = bytearray(old_mounts[0]).decode("utf-8")
        except Exception as e:
            print(f"Failed to get/parse mount point: {e}")
        return mount_point
        ...
        try:
            if mount_point is not None:
                ...
                unmount = fs.get_dbus_method(
                    "Unmount", dbus_interface="org.freedesktop.UDisks2.Filesystem"
                )
...
def main():
    ...
    if len(sticks) == 0:
        ...
        print(f"{len(sticks)} sticks found. Try unplugging one.")
    else:
