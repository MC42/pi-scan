...
class Camera:
    ...
    def log(self, message):
        errorlog.write(
            self.position + " camera: " + message + " -- self.message: " + self.message
        )
    ...
    def prepare(self):
        ...
        if self.isReady:
            ...
            try:
                ...
                self.log(
                    "Failed while preparing: PTPError: "
                    + str(e.args)
                    + "\n"
                    + traceback.format_exc()
                    + "\nPTP Traceback:\n"
    ...
    def prepare_zoom(self):
        ...
        self.device.lua_execute(
            "set_zoom(%d);sleep(250);" % self.calculate_zoom(), do_return=False
        )
    ...
    def prepare_whitebalance(self):
        ...
        self.device.lua_execute(
            "set_prop(require('propcase').WB_MODE, "
            + str(self.whitebalance)
            + ");sleep(50);",
            do_return=False,
        )
    ...
    def prepare_quality(self):
        ...
        self.device.lua_execute(
            "set_prop(require('propcase').QUALITY, 0);sleep(50);", do_return=False
        )

    def prepare_resolution(self):
        self.message = "Failed to set resolution"
        self.device.lua_execute(
            "set_prop(require('propcase').RESOLUTION, 0);sleep(50);", do_return=False
        )
    ...
    def unlockFocus(self):
        ...
        try:
            if self.is_connected():
                ...
                self.device.lua_execute(
                    "sleep(50);set_aflock(0);sleep(50);", do_return=False
                )
                success = True
            else:
            ...
            self.log(
                "Failed to unlock focus: PTPError: "
                + str(e.args)
                + "\n"
                + traceback.format_exc()
                + "\nPTP Traceback:\n"
            ...
            self.log(
                "Failed to unlock focus: " + str(e.args) + "\n" + traceback.format_exc()
            )
        return success
    ...
    def calculate_zoom(self):
        if self.zoom_steps is None:
            self.zoom_steps = self.device.lua_execute(
                "sleep(50); return get_zoom_steps()"
            )
        choice = "5"
        ...
        try:
            ...
            if self.isReady and self.is_connected():
                ...
                if data:
                    ...
                    self.device.lua_execute(
                        "sleep(50); enter_alt(); sleep(50);", do_return=False
                    )
                else:
    ...
    def shoot(self, options):
        ...
        self.device.lua_execute(
            "sleep(50); return rs_init(%s)" % options,
    ...
    def turnOff(self):
        try:
            if self.is_connected():
                self.device.lua_execute(
                    'sleep(50); post_levent_to_ui("PressPowerButton")', do_return=False
                )
        except chdkptp.PTPError as e:
            self.log(
                "Failed to power off: PTPError: "
                + str(e.args)
                + "\n"
                + traceback.format_exc()
                + "\nPTP Traceback:\n"
